...
const ProductReport = () => {
    ...
    const [endAtSearch, setEndAtSearch] = useState(1745989200000 + 30 * 24 * 60 * 60 * 1000); // 05/30/2025
    ...
    const getData = async (_startDate, _endDate) => {
        try {
            ...
            setStartAtSearch(Math.min(..._data.map(d => new Date(d.coupon_start_at).getTime())) - 24 * 60 * 60 * 1000);
            setEndAtSearch(Math.max(..._data.map(d => new Date(d.coupon_end_at).getTime())) + 24 * 60 * 60 * 1000);
            setData(_data);
        } catch(e) {
            alert(e.message);
        }
    ...
    const filteredData = data.filter(d => {
        ...
        return (codeSearch.trim() === "" ? true : d.coupon_code.toLowerCase().includes(codeSearch.toLowerCase()))
            && (typeSearch.trim() === "" ? true : d.coupon_type === parseInt(typeSearch))
            && (_endAt <= endAtSearch && startAtSearch <= _startAt);
    }).sort((a, b) => {
        let result = -1;
        if (sortKey.trim() === "") {
            return result;
                    ...
                    { Array.isArray(filteredData) && filteredData.length > 0 ? (
                        filteredData.map((d) => (
                            <tr key={d.coupon_id} className="bg-gray-100">
                                ...
                                <td className="p-2 text-center border">{ d.coupon_type === 0 ? `${d.coupon_value * 100}%` : `$${d.coupon_value.toFixed(2)}` }</td>
                                <td className="p-2 text-center border">{ d.coupon_type === 0 ? "%" : "$" }</td>
                                <td className="p-2 text-center border">{ new Date(d.coupon_start_at).toLocaleDateString("en-US") }</td>
                                <td className="p-2 text-center border">{ new Date(d.coupon_end_at).toLocaleDateString("en-US") }</td>
                                <td className="p-2 text-center border">{ d.coupon_order_count }</td>
